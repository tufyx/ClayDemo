...
class NewDoorViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    
    @IBOutlet weak var cancelButton: UIBarButtonItem!
    @IBOutlet weak var saveButton: UIBarButtonItem!
    @IBOutlet weak var accessList: UITableView!
    @IBOutlet weak var doorNameTextField: UITextField!
    @IBOutlet weak var doorDescriptionTextField: UITextField!
    
    // the data source for the accessList table view
    var accessLevels: [AccessLevel] = Utils.accessLevelList()
    
    // the selected level of the Door
    var selectedLevel: AccessLevel?
    
    // the delegate of this VC
    var delegate: DoorProtocol?
    ...
    func configureUI() {
        if (self.editedDoor != nil) {
            self.doorNameTextField.text = editedDoor!.doorName
            self.doorDescriptionTextField.text = editedDoor!.doorDescription
        }
    ...
    func tapView() {
        self.doorNameTextField.resignFirstResponder()
        self.doorDescriptionTextField.resignFirstResponder()
    }
    ...
    func configureBarButtons() {
        ...
        self.saveButton.action = "saveDoor"
        self.cancelButton.action = "dismissScene"
    }
    
    // validates the add/edit form
    // door name should not be empty
    // door description should not be empty
    // an access level should be selected
    func isFormValid(formData: [String:String]) -> Bool {
        let doorName = formData["doorName"]!
        let doorDescription = formData["doorDescription"]!
        var errorMessage = ""
        if doorName.isEmpty {
            errorMessage = "Fill in door name"
        } else if doorDescription.isEmpty {
            errorMessage = "Fill in door description"
    ...
    func saveDoor() {
        let doorName = self.doorNameTextField.text
        let doorDescription = self.doorDescriptionTextField.text
        
        if !self.isFormValid(["doorName": doorName, "doorDescription": doorDescription]) {
            return
        }
        
        if (self.editedDoor != nil) {
            self.editedDoor!.doorName = doorName
            self.editedDoor!.doorDescription = doorDescription
            self.editedDoor!.accessLevel = self.selectedLevel!
            self.delegate?.doorWasEdited(self.editedDoor!)
        } else {
            self.delegate?.doorWasCreated(Door(name: doorName, doorDescription: doorDescription, level: self.selectedLevel!))
        }
        self.dismissScene()
    }
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let level = self.editedDoor?.accessLevel {
            // and the current cell is the door's access level, set the accessory type to Checkmark
    ...
    func uncheckAllCells() {
        for index in 0...self.accessLevels.count {
            ...
            if let cell = self.accessList.cellForRowAtIndexPath(indexPath) as? AccessCell {
